...
try:
    ...
    print("Предупреждение: Flash Attention 3 недоступен (Blackwell sm_120?), используется резервный вариант torch sdpa")
    fa3 = None
    HAS_FA3 = False
...
class CausalSelfAttention(nn.Module):
    def __init__(self, config, layer_idx):
        ...
        self.c_qkv = nn.Linear(self.n_embd, (self.n_head + 2 * self.n_kv_head) * self.head_dim, bias=False)
    ...
    def forward(self, x, ve, cos_sin, window_size):
        ...
        k = qkv[..., q_size:q_size+k_size].view(B, T, self.n_kv_head, self.head_dim)
        v = qkv[..., q_size+k_size:].view(B, T, self.n_kv_head, self.head_dim)
